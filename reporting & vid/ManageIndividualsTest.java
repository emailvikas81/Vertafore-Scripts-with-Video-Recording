...
import com.vertafore.common.ScreenshotOnFailRule;
...
import com.vertafore.pageobjects.AddCommentLicensePage;
...
import com.vertafore.pageobjects.EditCommentLicensePage;
...
public class ManageIndividualsTest {
	...
	public static void setUp() throws Exception {
		G2_Common.APPLICATION_LOGS.info("Test setup starts...");
	...
	public static void tearDown() {
		...
		driver.quit();
		/*
		 * try { videoRecord.stopRecording(); } catch (Exception e) { // TODO
	...
	public void testA_addIndividual() {
		final GlobalPage global = PageFactory.initElements(driver,
				GlobalPage.class);
		final SearchIndividualsPage searchIndPage = PageFactory.initElements(
				driver, SearchIndividualsPage.class);
		final AddIndividualPage addIndPage = PageFactory.initElements(driver,
				AddIndividualPage.class);

		G2_Common.APPLICATION_LOGS.info("Uniquely generated number is : " + id);
		String lastName = Global_Common.getEnvironmentProperty("lastName") + id;
		String firstName = Global_Common.getEnvironmentProperty("firstName")
				+ id;
		...
		externalSystemID = addIndPage.addIndividual(lastName, firstName, sSN,
				cRD, nPN, custManaged, residentStateOne, uSG);
		if (global.taskResponse().equals("error")) {
			if (global.retrieveFailureReason().contains(
					"Individual SSN already exists.")) {
				testA_addIndividual(); // call the method again , if the SSN
										// already exists
			} else {
				G2_Common.APPLICATION_LOGS
	...
	public void testB_addresses() {
		...
		final EditAddressPage editAddrPage = PageFactory.initElements(driver,
				EditAddressPage.class);
		final CloneAddressPage cloneAddrPage = PageFactory.initElements(driver,
				CloneAddressPage.class);
		String aaAddressPurpose = Global_Common
				.getEnvironmentProperty("aa_AddressPurpose");
		String aaFromDate = Global_Common.getEnvironmentProperty("aa_FromDate");
		...
		String aaLineOne = Global_Common.getEnvironmentProperty("aa_LineOne");
		String aaCity = Global_Common.getEnvironmentProperty("aa_City");
		String aaCountry = Global_Common.getEnvironmentProperty("aa_Country");
		...
		addAddrPage.addAddress(aaAddressPurpose, aaFromDate, aaAddressType,
				...
				aaPostalCode);
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "Address-");
			fail("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
		}
		String data[] = { "", "", aaAddressType, aaAddressPurpose, "Active",
				"", aaLineOne, aaCity, aaState, aaPostalCode, aaCountry,
				aaFromDate, "", "" };
		global.validateTable(driver, "//*[@id='HG1']/tbody/tr", 3, data);

		// Edit Address
		String eaFromDate = Global_Common.getEnvironmentProperty("ea_FromDate");
		String eaPrivateResidence = Global_Common
				.getEnvironmentProperty("ea_PrivateResidence");
		String eaLineOne = Global_Common.getEnvironmentProperty("ea_LineOne");
		String eaCity = Global_Common.getEnvironmentProperty("ea_City");
		String eaCountry = Global_Common.getEnvironmentProperty("ea_Country");
		...
		String data2[] = { "", "", "Business", "Human Resource/Administration",
				"Active", "", eaLineOne, eaCity, eaState, eaPostalCode,
				eaCountry, eaFromDate, "", "" };
		...
		String caFromDate = Global_Common.getEnvironmentProperty("ca_FromDate");
		...
		String caLineOne = Global_Common.getEnvironmentProperty("ca_LineOne");
		String caCity = Global_Common.getEnvironmentProperty("ca_City");
		String caCountry = Global_Common.getEnvironmentProperty("ca_Country");
		...
		G2_Common.navigateTo(driver, "Clone");
		cloneAddrPage.cloneAddress(caAddressPurpose, caFromDate, caAddressType,
				caFinraSeqNo, caToDate, caLineOne, caCity, caCountry, caState,
				caPostalCode);
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
		...
		String data3[] = { "", "", "Business", caAddressPurpose, "Inactive",
				"", caLineOne, caCity, caState, caPostalCode, caCountry,
				caFromDate, caToDate, caFinraSeqNo };
		global.validateTable(driver, "//*[@id='HG1']/tbody/tr", 4, data3);
		// Add Comment in Address
		G2_Common.navigateTo(driver, "Add Comment");
		final AddCommentPage addCommentPage1 = PageFactory.initElements(driver,
				AddCommentPage.class);
		global.checkDataValidationErrors(dataValidationErrors3);
		addCommentPage1.addComment("AAD.", "Address");
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "Address-");
		...
		editCommentpage1.editComment("EAP.", "Address");
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "Address-");
	...
	public void searchIndividual() { // Search
										// Individual
										// using
										// Lastname,
										// FirstName
	...
	@Ignore
	@Test
	public void testC_additionalRegulatoryRequirements() {
		final GlobalPage global = PageFactory.initElements(driver,
				GlobalPage.class);
		...
		String addState = Global_Common.getEnvironmentProperty("addState");
		...
		editRegulatoryReqPage.editRegulatoryReq(editARRState, editARRStatus,
		...
		String data2[] = { "", "Fixed Annuity Product Training", "Training",
				"", editARRState, "", editARRStatus, "", editARRRequestedDate,
				"", "", "" };
		global.validateTable(driver, "//*[@id='HG1']/tbody/tr", 3, data2);
	...
	@Ignore
	@Test
	public void testD_employmentHistory() {
		final GlobalPage global = PageFactory.initElements(driver,
				GlobalPage.class);
		G2_Common.actionsNavigateTo(driver, "Employment History");
		final EmploymentHistoryPage empHistoryPage = PageFactory.initElements(
				driver, EmploymentHistoryPage.class);
		...
		String addenddate = Global_Common.getEnvironmentProperty("addToDate");
		String addcityname = Global_Common.getEnvironmentProperty("addcity");
		String addpostalcode = Global_Common
				.getEnvironmentProperty("addPostalCode");
		String addstatename = Global_Common.getEnvironmentProperty("addState");
		...
		addEmpHistoryPage.addEmploymentHistory(addemployer,
				addfinrasequencenumber, addpositionheld, addinvestmentrelated,
				addstartdate, addenddate, addcityname, addpostalcode,
				addstatename, addcountryname);
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "EmpHistory-");
		...
		String data1[] = { "", addemployer, addinvestmentrelated, addcityname,
				addstatename, addpostalcode, addcountryname, "06/11/2014",
				"11/27/2014", addpositionheld, addfinrasequencenumber };
	...
	public void testE_comments() {
		...
		commentPage1.clickAddComment();
		addCommentPage1.addComment("", "Individual"); // Mandatory to know what
														// level comments are
		...
		String data1[] = { "", "General                                  ",
				"12/17/2014", "Individual", "",
				"Add comment for the individuals here", };
		global.validateTable(driver,
				"//*[@id='ConfigureCommentForm']/table[2]/tbody/tr", 1, data1);
		...
		String data2[] = { "", "Inquiry                                  ",
				"11/26/2014", "Individual", "", "Editing the comment here." };
		global.validateTable(driver,
				"//*[@id='ConfigureCommentForm']/table[2]/tbody/tr", 1, data2);
	...
	public void testF_errorsAndOmissions() {
		...
		// Add E&O
		final ErrorsAndOmissionsPage eaoPage = PageFactory.initElements(driver,
				ErrorsAndOmissionsPage.class);
		String addStatus = Global_Common.getEnvironmentProperty("addStatus");
		...
		String addPolicy = Global_Common.getEnvironmentProperty("addPolicy");
		...
		final AddErrorsAndOmissionsPage addEAOPage = PageFactory.initElements(
				driver, AddErrorsAndOmissionsPage.class);
		global.checkDataValidationErrors(dataValidationErrors);
		if (global.validateCancelOp(pageHeader).equals("error")) {
			G2_Common.APPLICATION_LOGS
					.error("Test failed due to invalid page title. Check execution log. Expected title:   "
		...
		String data1[] = { "", addProviderName, addPolicy, addEffectiveDate,
				addExpirationDate, addStatus };
		...
		G2_Common.navigateTo(driver, "Edit");
		editEAOPage.editErrorsAndOmissions(updateStatus, updateEffectiveDate,
				updateExpirationDate, updateProviderName, updatePolicy,
		...
		String data3[] = { "", "General                                  ",
				"12/17/2014", "Errors & Omissions", "",
				"Add comment for the E&O here" };
		global.validateTable(driver,
				"//*[@id='ConfigureCommentForm']/table[2]/tbody/tr", 1, data3);
	...
	public void testG_externalSystemIdentifiers() {
		...
		String addEndDate = Global_Common.getEnvironmentProperty("addEndDate");
		String dataValidationErrors = Global_Common
		...
		esidPage.clickAddExternalsystemID();
		addESIDPage.addExternalSystemIdentifier(addType,
				addIdentificationInExternalSystem, addStartDate, addEndDate);
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "ExtSysID-");
		...
		String data1[] = {
				"    ",
				addType + "                                  ",
				addIdentificationInExternalSystem
						+ "                                ",
				addStartDate + "                               ",
				addEndDate + "                               " };
		String data2[] = { "    ", "System ID                                ",
				externalSystemID + "                                  ", "",
				"                                         " };
		...
		final EditExternalSystemIdentifierPage editESIDPage = PageFactory
				.initElements(driver, EditExternalSystemIdentifierPage.class);
		String updateType = Global_Common.getEnvironmentProperty("editType");
	...
	public void testI_adminSupervisor() {
		...
		String data1[] = { "", adminLastName + admiFirstName, "***-**-5555",
				"4444444444", "4444444444", "3459723", "", adminFromDate, "",
				adminSuperStatus };
		...
		editAdminSupervisorPage.editAdministrativeSupervisor(adminEditFromDate,
				adminEditSuperStatus);
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "AdminSupervisor-");
		...
		String data2[] = { "", adminLastName + admiFirstName, "***-**-5555",
				"4444444444", "4444444444", "3459723", "", adminEditFromDate,
				"", adminEditSuperStatus };
		global.validateTable(
				driver,
				"//form[@id='SupervisorForm']/table/tbody/tr[3]/td/table[3]/tbody/tr",
	...
	@Ignore
	@Test
	public void testJ_businessProcessesAndEvents() {

		final GlobalPage global = PageFactory.initElements(driver,
				GlobalPage.class);
		G2_Common.actionsNavigateTo(driver, "Business Processes and Events");
		...
		String data1[] = { "", "", addBusinessProcess, "", "", addBPEStatus,
				"", addBPEStart, "", addBPECommentText };
		global.validateTable(driver, "//*[@id='HG1']/tbody/tr", 3, data1);

		// Edit Business Process
		String bpeState = Global_Common.getEnvironmentProperty("bpeState");
		String bpeStatus = Global_Common.getEnvironmentProperty("bpeStatus");
		...
		String bpeComment = Global_Common.getEnvironmentProperty("bpeComment");
		String dataValidationErrors2 = Global_Common
		...
		G2_Common.navigateTo(driver, "Edit");
		final EditBusinessProcessPage editBPPage = PageFactory.initElements(
				driver, EditBusinessProcessPage.class);
		...
		final EditEventBusinessProcessEventPage editBPEventPage = PageFactory
				.initElements(driver, EditEventBusinessProcessEventPage.class);
	...
	public void testK_communicationTrackingSummary() {
		...
		final CommunicationTrackingSummaryPage ctPage = PageFactory
				.initElements(driver, CommunicationTrackingSummaryPage.class);
		// Add Communication Tracking
		// String fromCommunicationDate = Global_Common
		// .getEnvironmentProperty("fromCommunicationDate");
		// String communicationType = Global_Common
		// .getEnvironmentProperty("communicationType");
		...
		String actDueDate = Global_Common.getEnvironmentProperty("act_DueDate");
		...
		// communicationType, toCommunicationDate);
		String pageHeader = driver
				.findElement(By.cssSelector("td.bannerTitle")).getText();
		...
		String data1[] = { "", "User Automation",
				actDateOfCommunication + " " + actTimeOfCommunication + " AM",
				actCommunicationType, actSubjectOfCommunication, actDueDate,
				actCompletionDate, "" };
		global.validateTable(driver, "//form/table[2]/tbody/tr", 1, data1);
		...
		String ectDueDate = Global_Common.getEnvironmentProperty("ect_DueDate");
		...
		String data2[] = { "", "User Automation",
				ectDateOfCommunication + " " + ectTimeOfCommunication + " AM",
				ectCommunicationType, ectSubjectOfCommunication, ectDueDate,
				ectCompletionDate, "" };
		...
		viewCTPage.viewCommunicationTracking();// this is just view, so just
												// clicking return, can do some
												// validations.
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
	...
	public void testL_firmAssociation() {
		...
		global.checkDataValidationErrors(dataValidationErrors);
		addFAPage2.addFirmAssociation(assignmentStartDate, assignmentStatus,
				assignmentPersonnelRole);
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "FirmAssociation-");
		...
		String pageHeader1 = driver.findElement(
				By.cssSelector("td.bannerTitle")).getText();
		G2_Common.navigateTo(driver, "Edit");
		final EditFirmAssociationPage editFAPage = PageFactory.initElements(
				driver, EditFirmAssociationPage.class);
		if (global.validateCancelOp(pageHeader1).equals("error")) {
			G2_Common.APPLICATION_LOGS
					.error("Test failed due to invalid page title. Check execution log. Expected title:   "
							+ pageHeader1);
		...
		String data3[] = { "", "", "Stifel", "", "", "793", "3035823",
				"Active", "Firm", addPersonnelRole, "", addRoleStartDate, "",
				"Active", "" };
		global.validateTable(driver, "//*[@id='HG1']/tbody/tr", 4, data3);
		...
		String data4[] = { "", "", "Stifel", "", "", "793", "3035823",
				"Active", "Firm", "Test Defect", "", updateRoleStartDate, "",
				"Active", "", "Terminated" };
		global.validateTable(driver, "//*[@id='HG1']/tbody/tr", 4, data4);
		...
		String data6[] = { "", "General                                  ",
				"11/27/2014", "Producer Relationship", "",
				"Edit comment for the Firm Association here" };
		global.validateTable(driver,
				"//*[@id='ConfigureCommentForm']/table[2]/tbody/tr", 1, data6);
		...
		G2_Common.navigateTo(driver, "Edit");
		editCommentpage1.editComment("FA.", "Producer Relationship");
		// Please note the syntax -keyword followed by dot -hard-coded keyword
		...
		// String data7[] = { "", "General                                  ",
		// "11/27/2014", "Producer Relationship", "",
		// "Edit comment for the Firm Association here" };
		// global.validateTable(driver,
		// "//*[@id='ConfigureCommentForm']/table[2]/tbody/tr", 1, data7);
	...
	public void testM_jurisdictions() {
		...
		String pageHeader = driver
				.findElement(By.cssSelector("td.bannerTitle")).getText();
		final JurisdictionSummaryPage jsPage = PageFactory.initElements(driver,
				JurisdictionSummaryPage.class);
		...
		String data1[] = { "", "", addFirm, addJurisdiction,
				addRegistrationType, addStatus, addStatusDate, addFilingDate,
				"", addFinraStatus };
		global.validateTable(driver, "//*[@id='resultTableBody']/tr", 1, data1);
		...
		global.validateTable(driver, "//*[@id='resultTableBody']/tr", 1, data2);
		...
		G2_Common.navigateTo(driver, "Edit");
		editCommentpage1.editComment("JD.", "Jurisdictions");
		...
		String data4[] = { "", "General                                  ",
				"11/27/2014", "Jurisdiction", "",
				"Edit comment for the Jurisdiction here" };
		global.validateTable(driver,
				"//*[@id='ConfigureCommentForm']/table[2]/tbody/tr", 1, data4);
	...
	public void testQ_complianceSupervisor() {
		...
		final AddComplianceSupervisorFirstPage addCSFPage = PageFactory
				.initElements(driver, AddComplianceSupervisorFirstPage.class);
		...
		final AddcomplianceSupervisorSecondPage addCSSPage = PageFactory
				.initElements(driver, AddcomplianceSupervisorSecondPage.class);
		global.checkDataValidationErrors(dataValidationErrors);
		if (global.validateCancelOp(pageHeader).equals("error")) {
			G2_Common.APPLICATION_LOGS
					.error("Test failed due to invalid page title. Check execution log. Expected title:   "
		...
		String addIndividualRoleToComplainceSupervisor = Global_Common
		...
		addRoleCSSPage.addRoleToComplianceSupervisor(
				...
				addIndividualRoleToComplainceSupervisor, addRoleStartDate);
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "CompSupervisor-");
		...
		String data1[] = { "", "", "Last Name, 28apr", "***-**-5555", "", "",
				"", "", "Active", addSupervisorRoleToComplianceSupervisor,
				addIndividualRoleToComplainceSupervisor, addRoleStartDate, "",
				"Active" };
		...
		final EditRoleComplianceSupervisorPage editRoleCSSPage = PageFactory
				.initElements(driver, EditRoleComplianceSupervisorPage.class);
		G2_Common.navigateTo(driver, "Edit Role");
		if (global.validateCancelOp(pageHeader2).equals("error")) {
			G2_Common.APPLICATION_LOGS
					.error("Test failed due to invalid page title. Check execution log. Expected title:   "
		...
		G2_Common.navigateTo(driver, "Edit");
		editCSSPage.editComplianceSupervisor(editComplianceSupervisonStartDate,
	...
	public void testR_locationofConvenience() {
		...
		String alcLineOne = Global_Common.getEnvironmentProperty("alc_LineOne");
		String alcCity = Global_Common.getEnvironmentProperty("alc_City");
		String alcCountry = Global_Common.getEnvironmentProperty("alc_Country");
		...
		String data[] = { "", alcLineOne, alcCity, alcState, alcPostalCode,
		...
		String elcLineOne = Global_Common.getEnvironmentProperty("elc_LineOne");
		String elcCity = Global_Common.getEnvironmentProperty("elc_City");
		String elcCountry = Global_Common.getEnvironmentProperty("elc_Country");
		...
		final WebElement moveOverObject2 = driver.findElement(By
				.xpath("//div[@id='KF_ITEM_01']/img"));
		G2_Common.navigateToMenu(driver, moveOverObject2, "Edit");
		final EditLocOfConveniencePage editLOCPage = PageFactory.initElements(
				driver, EditLocOfConveniencePage.class);
		...
		String data1[] = { "", elcLineOne, elcCity, elcState, elcPostalCode,
				elcCountry, "", elcFromDate, "", elcDescription };
		global.validateTable(driver, "//*[@id='HG1']/tbody/tr", 7, data1);

		// Audit
		String id = Global_Common.generateRandomNumber(4);
		G2_Common.APPLICATION_LOGS.info("Uniquely generated License No. is  : "
				+ id);
		...
		final ViewAuditDashboardPage viewAuditPage = PageFactory.initElements(
				driver, ViewAuditDashboardPage.class);
		final AddAuditPage addAuditPage = PageFactory.initElements(driver,
				AddAuditPage.class);
		viewAuditPage.viewAudit();// this is just view, so just clicking return,
	...
	@Ignore
	@Test
	public void testO_licenseAndQualification() {
		...
		String alLicenseNumber = Global_Common
				.getEnvironmentProperty("al_LicenseNumber") + id;
		...
		final SummaryOfLicensesQualificationsPage slaqPage = PageFactory
				.initElements(driver, SummaryOfLicensesQualificationsPage.class);
		slaqPage.addLicense();
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
		...
		laqPage.addLicense(alState, alLicenseType, alStatus,
				alOriginalIssueDate, alPhysicalCopyReceived, alEffectiveDate,
				alRenewalStatus, alLicenseNumber, alExpirationDate,
				alQualification, ceReviewPeriodStartDate,
				ceReviewPeriodEndDate, ceReviewPeriodStatus,
				cerEthicsStartDate, cerEthicsRequiredHours, cerEthicsStatus,
		...
		String data[] = { "", "", alState, alLicenseType, "Yes",
				...
				alExpirationDate, "12/18/2014", "None/Record Only", "No" };
		global.validateTable(driver, "//*[@id='HG1']/tbody/tr", 3, data);
		...
		elaqPage.editLicense(elOriginalIssueDate, elLicenseNumber, elEffective,
				elExpirationDate, elPhysicalCopy, elRenewalStatus);
		// G2_Common.navigateTo( driver, "Edit");
		if (global.taskResponse().equals("error")) {
			if (global.retrieveFailureReason().contains(
		...
		String data8[] = { "", "", "CA - California",
				...
				"Active", elEffective, "", elExpirationDate, "12/18/2014",
				"None/Record Only", "No" };
		...
		G2_Common.navigateToMenu(driver, moveOverObject2, "Add Restriction");
		...
		final AddRestrictionPage arlaqPage = PageFactory.initElements(driver,
				AddRestrictionPage.class);
		global.checkDataValidationErrors(dataValidationErrors2);
		...
		// Edit Restriction
		String erEndDate = Global_Common.getEnvironmentProperty("er_EndDate");
		...
		final EditRestrictionPage erlaqPage = PageFactory.initElements(driver,
				EditRestrictionPage.class);
		...
		AddCommentLicensePage addCommentPage1 = PageFactory.initElements(
				driver, AddCommentLicensePage.class);
		addCommentPage1.addComment("L&Q.", "Licenses & Qualifications");
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "L&Q-");
		...
		// edit Comment
		WebElement moveOverObject5 = driver.findElement(By
				.xpath(".//*[@id='KF_ITEM_0']/img"));
		G2_Common.navigateToMenu(driver, moveOverObject5, "Edit");
		EditCommentLicensePage editCommentPage2 = PageFactory.initElements(
				driver, EditCommentLicensePage.class);
		editCommentPage2.editComment("L&Q.", "Licenses & Qualifications");
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "L&Q-");
	...
	@Test
	public void testP_AddAppointment() {
		GlobalPage global = PageFactory.initElements(driver, GlobalPage.class);

		final SearchIndividualsPage searchIndPage = PageFactory.initElements(
				driver, SearchIndividualsPage.class);
		searchIndPage.navigateToManageIndividuals();
		searchIndPage.searchIndividuals("LastName4612", "FirstName4612");
		G2_Common.navigateTo(driver, "Affiliations & Appointments");
		...
		addAPPPage.addSingleStateAppointment();
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "A&A-");
			fail("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
		}
		...
		// G2_Common.navigateToMenu(driver, Mouseover, "Add");
		// AddAPPPage.AddSingleStateAppointment();
		// if (Global.TaskResponse().equals("error")) {
		// fail("Test failed due to Data Error: "
		// + Global.retrieveFailureReason());
		...
		AddCommentPage addCommentPage1 = PageFactory.initElements(driver,
				AddCommentPage.class);
		addCommentPage1.addComment("App.", "Appointment"); // Please note the
															// syntax -
		...
		affAddResPage.addRestrictionToAffiliation();
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "A&A-");
		...
		/*
		 * Termination option will be displayed if and only if the Status is
		 * active and restriction is "No" So I have disabled Because we are
		 * creating it with the status pending.
		 */

		// G2_Common.navigateTo(driver, "Termination");
		// TerminateAffiliationPage TAffPage= PageFactory.initElements(
		// driver, TerminateAffiliationPage.class);
		// TAffPage.TerminateAffiliation();
		// if (Global.TaskResponse().equals("error")) {
		// fail("Test failed due to Data Error: "
		// + Global.retrieveFailureReason());
		...
		affViewHisPage.viewHistoryOfAffiliation();
	...
	public void testS_heightenedSupervison() {
		...
		final SearchIndividualsPage searchIndPage = PageFactory.initElements(
				driver, SearchIndividualsPage.class);
		searchIndPage.navigateToManageIndividuals();
		searchIndPage.searchIndividualsForHeightenedSupervision(identifiers,
				identifierValue);
		...
		hsPage.clickOnAddHeightenedSupervision();
		addHSPage.addHeightenedSupervison(fromDate, toDate, supervisionReason,
				heightenedSupervisionSupervisor);
		if (global.taskResponse().equals("error")) {
			G2_Common.APPLICATION_LOGS.error("Test failed due to Data Error: "
					+ global.retrieveFailureReason());
			G2_Common.captureScreenShot(driver, "HeightenedSuper-");
