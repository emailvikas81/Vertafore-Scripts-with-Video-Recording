...
public class AddAppointmentSubActionPage<IWebElement> {
	...
	public AddAppointmentSubActionPage(WebDriver driver) {
		this.driver = driver;

	}

	public void addSingleStateAppointmentSubAction(String company,
			String state, String appointmentType, String billingCode,
			String agentCode, String agencyCode, String producerType,
			String ProcessingInstructions, String aaAPIDisabled) {
		String firstLetter = String.valueOf(company.charAt(0));
		G2_Common.updateField(driver, this.companyTextBox, firstLetter);
		...
		if (this.appointmentTypeCheckBox.isDisplayed()) { // put a explicit
															// wait here
		...
		if (!state.equals("AK - Alaska")) // Hard coding the data because
											// this case is valid for this
											// one only.
		{
	...
	public String selectStateDropDown(String state) {
		...
		System.out.println("1st:" + se.getFirstSelectedOption().getText());
		if (!se.getFirstSelectedOption().getText().equals(state)) {
			...
			System.out.println("2nd:" + se.getFirstSelectedOption().getText());
